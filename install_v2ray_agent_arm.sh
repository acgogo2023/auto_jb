...
checkSystem() {
    ...
    elif { [[ -f "/etc/issue" ]] && grep -qi "debian" /etc/issue; } || { [[ -f "/proc/version" ]] && grep -qi "debian" /proc/version; } || { [[ -f "/etc/os-release" ]] && grep -qiE "ID=debian|ID=armbian|ID_LIKE=.*debian" /etc/os-release; }; then
        release="debian"
