...
updateV2RayAgent() {
    ...
    echoContent green "\n ---> 更新完毕"
    echoContent yellow " ---> 请手动执行[v2ray]打开脚本"
    ...
    if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2ray-agent" ]] && grep <"$HOME/install.sh" -q "作者:mack-a"; then
        ...
        if [[ -d "/usr/bin/" ]]; then
            if [[ ! -f "/usr/bin/v2ray" ]]; then
                ln -s /etc/v2ray-agent/install.sh /usr/bin/v2ray
                chmod 700 /usr/bin/v2ray
                v2rayType=true
            fi

            rm -rf "$HOME/install.sh"
        elif [[ -d "/usr/sbin" ]]; then
            if [[ ! -f "/usr/sbin/v2ray" ]]; then
                ln -s /etc/v2ray-agent/install.sh /usr/sbin/v2ray
                chmod 700 /usr/sbin/v2ray
                v2rayType=true
            fi
            rm -rf "$HOME/install.sh"
        fi
        if [[ "${v2rayType}" == "true" ]]; then
            echoContent green "快捷方式创建成功，可执行[v2ray]重新打开脚本"
        fi
    fi
}
